...
const NavigationMenu = <T extends ValidComponent = 'ul'>(props: PolymorphicProps<T, NavigationMenuProps<T>>) => {
	...
	return (
		<NavigationMenuPrimitive.Root gutter={6} class={cn('group/menu flex w-max flex-1 list-none items-center justify-center data-[orientation=vertical]:flex-col [&>li]:w-full', local.class)} {...others}>
			{local.children}
...
type NavigationMenuViewportProps<T extends ValidComponent = 'li'> = NavigationMenuPrimitive.NavigationMenuViewportProps<T> & { class?: string | undefined }
...
type NavigationMenuLinkProps<T extends ValidComponent = 'a'> = NavigationMenuPrimitive.NavigationMenuItemProps<T> & { class?: string | undefined }
...
const NavigationMenuLink = <T extends ValidComponent = 'a'>(props: PolymorphicProps<T, NavigationMenuLinkProps<T>>) => {
	...
	return (
		<NavigationMenuPrimitive.Item
			class={cn('block select-none space-y-1 rounded-md p-3 leading-none no-underline outline-none transition-colors  hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground', local.class)}
			{...others}
		/>
	)
}

type NavigationMenuLabelProps<T extends ValidComponent = 'div'> = NavigationMenuPrimitive.NavigationMenuItemLabelProps<T> & { class?: string | undefined }
...
type NavigationMenuDescriptionProps<T extends ValidComponent = 'div'> = NavigationMenuPrimitive.NavigationMenuItemDescriptionProps<T> & { class?: string | undefined }
