...
const NumberFieldGroup: Component<ComponentProps<'div'>> = (props) => {
	...
	return <div class={cn('relative rounded-md focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2', local.class)} {...others} />
}
...
const NumberFieldLabel = <T extends ValidComponent = 'label'>(props: PolymorphicProps<T, NumberFieldLabelProps<T>>) => {
	...
	return <NumberFieldPrimitive.Label class={cn('text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70', local.class)} {...others} />
}
...
const NumberFieldIncrementTrigger = <T extends ValidComponent = 'button'>(props: PolymorphicProps<T, NumberFieldIncrementTriggerProps<T>>) => {
	const [local, others] = splitProps(props as NumberFieldIncrementTriggerProps, ['class', 'children'])
	return (
		<NumberFieldPrimitive.IncrementTrigger class={cn('absolute right-1 top-1 inline-flex size-4 items-center justify-center', local.class)} {...others}>
			{local.children ?? (
...
const NumberFieldDecrementTrigger = <T extends ValidComponent = 'button'>(props: PolymorphicProps<T, NumberFieldDecrementTriggerProps<T>>) => {
	const [local, others] = splitProps(props as NumberFieldDecrementTriggerProps, ['class', 'children'])
	return (
		<NumberFieldPrimitive.DecrementTrigger class={cn('absolute bottom-1 right-1 inline-flex size-4 items-center justify-center', local.class)} {...others}>
			{local.children ?? (
				<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' class='size-4'>
					<path d='M6 9l6 6l6 -6' />
				</svg>
...
const NumberFieldDescription = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, NumberFieldDescriptionProps<T>>) => {
...
const NumberFieldErrorMessage = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, NumberFieldErrorMessageProps<T>>) => {
...
export { NumberField, NumberFieldGroup, NumberFieldLabel, NumberFieldInput, NumberFieldIncrementTrigger, NumberFieldDecrementTrigger, NumberFieldDescription, NumberFieldErrorMessage }
