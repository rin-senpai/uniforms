...
const DropdownMenuSubTrigger = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, DropdownMenuSubTriggerProps<T>>) => {
	...
	return (
		<DropdownMenuPrimitive.SubTrigger class={cn('flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none focus:bg-accent data-[state=open]:bg-accent', props.class)} {...rest}>
			{props.children}
...
const DropdownMenuSubContent = <T extends ValidComponent = 'div'>(props: PolymorphicProps<T, DropdownMenuSubContentProps<T>>) => {
	...
	return (
		<DropdownMenuPrimitive.SubContent
			class={cn('z-50 min-w-32 origin-[var(--kb-menu-content-transform-origin)] overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-md animate-in', props.class)}
			{...rest}
		/>
	)
}
