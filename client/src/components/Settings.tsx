...
const Settings = () => {
    ...
    return (
        <div class="flex flex-row">
            <div class="flex flex-col">
                <div class="flex-col">
                    <For each={["Account", "Form AutoFill", "Notification Triggers"]}>
                        {(elem) => (
                        <div class="m-4">
                            <Button class="w-full" onclick={(e:Event) => setField(`${elem}`)}>{elem}</Button>
                        </div>
                        )}
                    </For>
                </div>
                <div class="m-4">
                        <Button class="w-full" onclick={(e:Event) => setField('DeleteAcc')}>Delete Account</Button>
                    </div>
                <div class="m-4">
                        <Button class="w-full">Log out</Button>
                </div>
            </div>
            { field() == 'Account' ? <div class="flex flex-row flex-grow">
            ...
            </div> : field() == 'Notification Triggers' ? <div class="flex flex-row flex-grow">
                <div class="flex flex-col w-64 flex-grow p-3">
                    ...
                    <For each={["Apple", "Orange", "Watermelon"]}>
                        {(elem) => (
                        <p class="my-2 w-full p-3 bg-gray-300 text-left rounded-xl">{elem}</p>
                        )}
                    </For>
                </div>
            </div> : null}
